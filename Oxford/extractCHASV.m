...
%% Chromatic ASV parameters

patchRadius = 5; % radius of patches around the keypoints
windowWidth = int16(1); % width of the gaussian blur window
numColorBins = 16; % number of color bins
binsScale = 32; % height of the bins of color histograms
...
%         d_asv = vl_asvcovdet(im1, opt, f, des, isInter);
        path = ['./imageFD/DoG/',num2str(i),'/',num2str(j)];
        load([path,'/ASV']);
        ...
        for k=1:size(f,2)
            ...
            for idx=1:size(patch,1)
                for jdx=1:size(patch,2)
                    ...
                    saturation = patch(idx,jdx,2);
                    value = patch(idx,jdx,3);
                    N(index) = N(index) + saturation * (-4 * value * (value - 1));
                end
            end
            N = sqrt(256 * N);
            smoothedVector = conv([N(end-halfWidth+1:end),N,N(1:halfWidth)], gaussFilter, 'same');
            if max(smoothedVector) > 0
                smoothedVector = binsScale * smoothedVector / max(smoothedVector);
            end
            N = floor(smoothedVector(halfWidth+1:end-halfWidth));

            colorDescriptor(:,k) = N;
        end
        d_chasv = vertcat(d_asv, colorDescriptor);
        ...
        save([nameF,'/CHASV'],'f','d_chasv','opt')
        t = toc;
