...
for i = 1:8
    for j = 1:6
        ...
        for k=1:size(f,2)
            ...
            for idx=1:size(patch,1)
                for jdx=1:size(patch,2)
                    newPoint = iA * [idx - patchRadius - 1; jdx - patchRadius - 1];
                    g = mvnpdf(newPoint);
                    index = floor(patch(idx,jdx,1)*numColorBins + 1);
                    N(index) = N(index) + patch(idx,jdx,2) * patch(idx,jdx,3) * g;
                end
            end
        ...
        d_ecasv = vertcat(d_asv, colorDescriptor);
        ...
        save([nameF,'/ECASV'],'f','d_ecasv','opt')
        t = toc;
